...
import numpy as np
import math
from utils.loss import multiple_label_aoc_loss
...
def get_troika(train_dataset, config, device, prompt_template="a photo of x x"):
    ...
    model = Troika(config, attributes=attributes, classes=classes, offset=offset).cuda()
...
class Troika(nn.Module):
    ...
    def construct_soft_prompt(self):
        '''
        Initial the token id/ word embedding/ 3 prompt embedding
        '''
    ...
    def loss_calu(self,predict,target):
        ...
        _, batch_attrs, batch_obj, batch_target = target
        comp_logits, attr_logits, obj_logits = predict
        batch_attrs = batch_attrs.cuda()
        batch_obj = batch_obj.cuda()
        batch_target = batch_target.cuda()
        loss_comp = loss_fn(comp_logits, batch_target)
        loss_attr = loss_fn(attr_logits, batch_attrs)
    ...
    def multi_attr_loss_calu(self, predict, target,pair_positive_weight=None,attr_positive_weight=None):
        ...
        loss = multiple_label_aoc_loss(predict=[predict[1],predict[2],predict[0]],
                                       target=target,
    ...
    def logit_infer(self, predict, pairs):
        ...
        comp_logits = F.softmax(comp_logits,dim=-1)
    ...
    def encode_primitive_text(self):
    ...
    def save_params(self,config,epoch,save_name=None):
        ...
        with torch.no_grad():
            if save_name!=None:
                model_path = os.path.join(config.save_path,save_name)
            elif epoch:
                model_path = os.path.join(
                    config.save_path, f"epoch_{epoch}.pt"
                )
            else:
                model_path = os.path.join(
                    config.save_path, "model.pt"
                )
