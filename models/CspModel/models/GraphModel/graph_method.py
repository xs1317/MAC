...
class GraphFull(nn.Module):
    def __init__(self, dset, args, element_embedding):
        ...
        if args.gcn_type == 'gcn':
            self.gcn = GCN(adj_matrix, args.emb_dim, args.emb_dim, hidden_layers)
        else:
            self.gcn = GCNII(adj_matrix, args.emb_dim, args.emb_dim, args.hidden_dim, args.gcn_nlayers, lamda = 0.5, alpha = 0.1, variant = False)
