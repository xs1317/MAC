...
class GraphPromptInterface(CLIPInterface):
    ...
    def init_embeddings(self, element_embedding):
        ...
        composition_embeds = get_compositional_embeddings(element_embedding, self.train_ds.pairs)
    ...
    def encode_text_for_open(self,pair_attr_obj_idx):
        # ------------------------------------------------
        # step 1: get all embedding of pair attr and obj
        # ------------------------------------------------
        # use graph model on this 
        pair_txt_emb = self.construct_pair_txt_emb(pair_attr_obj_idx)
        # pair_txt_emb = torch.randn([1262,10,768]).to(self.device)
        ...
        return norm_pair_feat

    def forward_for_open(self, batch_img,idx, norm_pair_feat):
        # ------------------
        # normalized img
        # ------------------
        if isinstance(batch_img,list):
