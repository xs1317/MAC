...
from models.CGE.image_extractor import get_image_extractor
import torch.optim as optim
import os

device = 'cuda' if torch.cuda.is_available() else 'cpu'

def get_gce(dataset, args, device):
    image_extractor = None
    is_open = False
    graph_model = GraphFull(dataset,args)
    ...
    model = GCE(graph_model,image_extractor,args)
...
class GCE(nn.Module):
    def __init__(self,graph,image_extractor,config=None):
    ...
    def forward(self, data, idx=None,text_features=None):

        data = [d.to(device) for d in data]
        
        data[0] = self.image_extractor(data[0])


        loss , predict = self.graph(data)

        return predict
    

    def encode_text_for_open(self,pairs):
        current_embedddings = self.graph.gcn(self.graph.embeddings)

        pair_embeds = current_embedddings[self.graph.num_attrs+self.graph.num_objs:self.graph.num_attrs+self.graph.num_objs+self.graph.num_pairs,:].permute(1,0)
        return pair_embeds
    

    def forward_for_open(self,data,idx, text_feats):
        data = [d.to(device) for d in data]
        
        data[0] = self.image_extractor(data[0])

        predict = torch.matmul(data[0],text_feats)
