import threading
import requests
import json
import io

import os
from PIL import Image

def split_dict(dict,split_num):
    dict_length = len(dict)
    split_size = dict_length // split_num + (dict_length % split_num > 0)
    result = []

    for i in range(0,dict_length,split_size):
        result.append( [dict[k] for k in range(i,min(i+split_size,len(dict)))] )

    return result


# down load task
class Downloader(threading.Thread):
    def __init__(self, url_list):
        super().__init__()
        self.url_list = url_list
        self.path = './images'
        self.fail_list = []
        #self.all_download_url = []

    def run(self):
        for item in self.url_list:
            

            image_path = item['image']
            image_url = item['url']
            self.download(image_path,image_url)

    def download(self,image_path,image_url):
        
        image_name = image_path.split('/')[-1]
        directory = os.path.dirname(image_path)
        directory = self.path + directory
        if not os.path.exists(directory):
            os.makedirs(directory)

                                
        image_path = self.path + image_path

        if not os.path.exists(image_path):
            try:
                # overlook the existed images
                image = requests.get(image_url,timeout=5)
                if image.status_code == 200:
                    with Image.open(io.BytesIO(image.content)) as image_from_web:
                        try:
                            print(
                                f"[INFO] {image_name} \t Image saved at: {image_path}   [url]: {image_url}  ")
                            image_from_web.save(image_path)
                        except OSError:
                            rgb_im = image_from_web.convert('RGB')
                            rgb_im.save(image_path)
                        image_from_web.close()
            except Exception as e:
                print("[ERROR] Download failed: ", image_name,image_url)
                self.fail_list.append(f"{image_name} : {image_url}")
                pass



if __name__ == "__main__":
    url_json_path ='./metadata_multi_attr_split_final.json'

    url_dict = {}
    with open(url_json_path,'r') as f:
        url_dict = json.load(f)


    # Split the URLs evenly among threads
    num_threads = 1
    
    split_url_dict = split_dict(url_dict,num_threads)


    # downloader = Downloader(split_url_dict[0])
    # downloader.run()
    threads = []
    for chunk in  split_url_dict:
        downloader = Downloader(chunk)
        downloader.start()
        threads.append(downloader)
        

    # Wait for all threads to finish
    for thread in threads:
        thread.join()

    with open('fail.log','w') as f:
        for d in threads:
            for fail_url in d.fail_list:
                f.write(fail_url+'\n')